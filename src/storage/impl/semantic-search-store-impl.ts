...
export class SemanticSearchStoreImpl implements SemanticSearchStore {
  ...
  async upsertEmbedding(input: {
    ...
    await this.db.prisma.memoryEmbedding.upsert({
      ...
      create: {
        ...
        embedding: input.embedding, model: input.model, dimensions: input.dimensions,
  ...
  async getEmbedding(entityType: string, entityId: string): Promise<{
  ...
  async searchByEmbedding(queryEmbedding: number[], opts: {
