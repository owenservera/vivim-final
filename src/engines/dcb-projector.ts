...
const LAYER_ORDER: LayerType[] = [
  ...
  'L5JitContext', 'L6RecentHistory', 'L7UserQuery',
];
...
function layerHeader(layer: LayerType): string {
  switch (layer) {
    case 'L0Identity':       return 'Who I am';
    case 'L1GlobalPrefs':    return 'Preferences';
    case 'L2Topic':           return 'Relevant background';
    case 'L3Entity':          return 'Key entities';
    case 'LpProjectState':   return 'Project state';
    case 'LdDecisions':      return 'Decisions';
    case 'L4Conversation':    return 'Conversation arc';
    case 'L5JitContext':      return 'Recent across apps';
    case 'L6RecentHistory':   return 'Recent messages';
    case 'L7UserQuery':       return 'My query';
  }
}
...
export function project(dcb: DynamicContextBundle, surface: Surface): string {
  switch (surface) {
    case 'inject_prompt':  return projectInject(dcb);
    case 'system_message': return projectSystem(dcb);
    case 'capsule':        return projectCapsule(dcb);
    case 'panel_card':    return projectCard(dcb);
  }
}
...
function sortedItems(items: LayerItem[]): LayerItem[] {
  return [...items].sort(
    (a, b) =>
      b.confidence * recencyDecay(b.recencySecs) -
      a.confidence * recencyDecay(a.recencySecs),
  );
}
...
function projectInject(dcb: DynamicContextBundle): string {
  ...
  if (dcb.projectId) {
    const providers = [...new Set(dcb.layers
      .filter((l) => l.provenance.provider)
...
function projectCapsule(dcb: DynamicContextBundle): string {
  ...
  if (trimmed.length > 0) {
    ...
    for (const item of trimmed) {
      parts.push(`- ${truncate(item.text, 80)} (~${item.tokenCost} tokens)  ^${item.provenance.label}`);
    }
    parts.push('');
  }
