...
import { P2P_DEFAULTS, P2P_PROTOCOLS } from '../../lib/tunnel-shared/constants.js'
  ...
  registerHandler(): void {
    this.node.handle(P2P_PROTOCOLS.FILE_SYNC, async (stream: Stream, _connection: Connection) => {
      ...
      try {
        ...
        const requestStr = new TextDecoder().decode(
          new Uint8Array(
            chunks.reduce((acc, c) => acc + c.length, 0) ? chunks.flatMap((c) => [...c]) : [0],
          ),
        )
  ...
  async sendFile(peerId: string, filePath: string, fileName: string): Promise<void> {
    ...
    try {
      ...
      if (verifyChunks.length > 0) {
        const completeStr = new TextDecoder().decode(
          new Uint8Array(verifyChunks.flatMap((c) => [...c])),
        )
