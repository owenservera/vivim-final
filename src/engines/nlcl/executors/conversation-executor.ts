...
export class ConversationExecutor implements CommandExecutor {
    ...
    try {
      switch (intent.intent) {
        case 'conversation.create':
          return await this.create(intent, ctx, traceId, start)
        case 'conversation.list':
          return await this.list(intent, ctx, traceId, start)
        case 'conversation.send':
          return await this.send(intent, ctx, traceId, start)
        case 'conversation.switch':
          return await this.switchProvider(intent, ctx, traceId, start)
        case 'conversation.messages':
          return await this.messages(intent, ctx, traceId, start)
        default:
  ...
  private async create(
    ...
    const conv = await this.conversationManager!.createConversation(providerId, title)
    return {
      ok: true,
  ...
  private async list(
    ...
    const convs = await this.conversationManager!.getMessages as unknown
    // Use store directly through conversationManager's store
    const store = (this.conversationManager as unknown as { store: { listConversations?: (limit: number) => Promise<unknown[]> } }).store
  ...
  private async send(
    ...
    const result = await this.conversationManager!.send(conversationId, message)
    return {
      ok: result.ok,
      intent: intent.intent,
  ...
  private async switchProvider(
    ...
    const conv = await this.conversationManager!.createConversation(providerId, `Switched to ${providerId}`)
    return {
      ok: true,
      intent: intent.intent,
      output: { conversationId: conv.id, providerId },
  ...
  private async messages(
    ...
    const messages = await this.conversationManager!.getMessages(conversationId, { limit })
    return {
      ok: true,
      intent: intent.intent,
