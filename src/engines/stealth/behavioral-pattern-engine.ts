...
export class BehavioralPatternModule implements StealthModule {
  ...
  async apply(config: Record<string, unknown>, ctx: StealthContext): Promise<void> {
    this.config = {
      ...
      betweenActionsMinMs: (config.betweenActionsMinMs as number) ?? DEFAULT_CONFIG.betweenActionsMinMs,
      betweenActionsMaxMs: (config.betweenActionsMaxMs as number) ?? DEFAULT_CONFIG.betweenActionsMaxMs,
      readingPauseProbability: (config.readingPauseProbability as number) ?? DEFAULT_CONFIG.readingPauseProbability,
