...
import { createHttpAbortFrame, createHttpResponseFrame, encodeFrame } from './frame-protocol.js'
import type { PendingRequest, TunnelMetrics } from './types.js'
...
export class RequestHandler {
  ...
  async handleRequest(frame: HttpRequestFrame): Promise<void> {
    ...
    this.metrics.bytesIn += JSON.stringify(frame).length
    ...
    try {
      ...
      const localReq: LocalServerRequest = {
        ...
        body: frame.body ? Buffer.from(frame.body, 'base64').toString('utf-8') : null,
  ...
  private sendResponse(
    ...
    this.metrics.latencyMs = this.updateAverageLatency(duration)
  ...
  private updateAverageLatency(newLatency: number): number {
    const total = this.metrics.requestsHandled
    const prev = this.metrics.latencyMs ?? 0
