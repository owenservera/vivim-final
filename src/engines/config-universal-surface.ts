...
export class ConfigUniversalSurface {
  ...
  listScopes(): ConfigScope[] {
    return [
      ...
      { id: 'capability', description: 'Capability configuration points', source: 'UnifiedCapabilityRegistry' },
      ...
      { id: 'autoheal', description: 'AutoHealPolicy for OracleActuator', source: 'OracleActuator', schema: AUTOHEAL_SCHEMA },
      { id: 'nlcl', description: 'NLCL resolver configuration', source: 'NLCLEngine', schema: NLCL_SCHEMA },
      { id: 'stealth', description: 'Stealth profile configuration', source: 'StealthManager', schema: STEALTH_SCHEMA },
      ...
      { id: 'canvas:layer', description: 'Canvas layer definition bindings', source: 'CanvasRegistry' },
  ...
  get(scope: string, key: string): ConfigValue | null {
    ...
    return cached ? { scope, key, value: cached, updatedAt: Date.now() } : { scope, key, value: null, updatedAt: 0 }
  }
  ...
  set(scope: string, key: string, value: unknown): ConfigValue {
    ...
    if (scopeInfo?.schema) {
      try {
        ...
        throw new Error(`Invalid config value for ${scope}.${key}: ${err instanceof Error ? err.message : String(err)}`)
      }
    }
    ...
    this.deps.eventBus.emit({ type: 'config:changed', engineId: scope, actor: 'cli', ...(value as Record<string, unknown>) })
