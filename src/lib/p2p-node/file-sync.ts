...
import { P2P_DEFAULTS, P2P_PROTOCOLS } from '../tunnel-shared/constants.js'
...
export class FileSyncHandler extends EventEmitter {
  ...
  registerHandler(): void {
    ...
    ;(this.node as any).handle(P2P_PROTOCOLS.FILE_SYNC, async (data: any) => {
      ...
      try {
        ...
        const request: FileSyncRequest = JSON.parse(new TextDecoder().decode(requestChunk))
  ...
  async sendFile(peerId: string, filePath: string, fileName: string): Promise<void> {
    ...
    try {
      ...
      const accept: FileSyncAccept = JSON.parse(new TextDecoder().decode(acceptChunk))
      ...
      if (completeChunk) {
        const complete: FileComplete = JSON.parse(new TextDecoder().decode(completeChunk))
