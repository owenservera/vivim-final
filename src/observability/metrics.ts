...
export class MetricsStore {
  ...
  observeHistogram(name: string, labels: Record<string, string>, value: number): void {
    const _key = this.labelKey(labels)
...
function registerDefaultMetrics(store: MetricsStore): void {
  // Fleet metrics
  store.register({
    name: 'chrome_fleet_size',
    help: 'Number of Chrome slaves by state',
    type: 'gauge',
    labels: ['state'],
  })
  store.register({
    name: 'chrome_slave_cdp_roundtrip_ms',
    help: 'CDP command round-trip latency',
    type: 'histogram',
  })
  store.register({
    name: 'chrome_circuit_state',
    help: 'Circuit breaker state (0=closed, 1=half, 2=open)',
    type: 'gauge',
    labels: ['slaveId'],
  })
  store.register({
    name: 'chrome_pool_leases',
    help: 'Active pool leases by provider',
    type: 'gauge',
    labels: ['provider'],
  })
  store.register({
    name: 'chrome_scheduler_queue_depth',
    help: 'Scheduler queue depth',
    type: 'gauge',
    labels: ['slaveId', 'queue'],
  })
  store.register({
    name: 'chrome_spawn_total',
    help: 'Total spawn attempts',
    type: 'counter',
    labels: ['result'],
  })
  store.register({
    name: 'chrome_health_probe_total',
    help: 'Total health probes',
    type: 'counter',
    labels: ['result'],
  })
  store.register({
    name: 'chrome_resource_pressure',
    help: 'Host resource pressure',
    type: 'gauge',
    labels: ['resource'],
  })
  store.register({
    name: 'chrome_concurrency_limit',
    help: 'Current concurrency limit',
    type: 'gauge',
  })
