...
export interface TextNode { type: 'text'; value: string; marks?: Mark[] }
...
export interface EmphNode { type: 'emphasis'; children: PhrasingContent[] }
export interface StrongNode { type: 'strong'; children: PhrasingContent[] }
export interface DeleteNode { type: 'delete'; children: PhrasingContent[] }
export interface InlineCodeNode { type: 'inlineCode'; value: string }
export interface LinkNode { type: 'link'; url: string; title?: string; children: PhrasingContent[] }
export interface ImageNode { type: 'image'; url: string; alt?: string; title?: string }
export interface BreakNode { type: 'break' }
export interface MathNode { type: 'math'; value: string }
export interface WidgetNode { type: 'widget'; kind: string; props: Record<string, unknown> }
export interface MentionNode { type: 'mention'; id: string }
...
export interface ParagraphNode { type: 'paragraph'; children: PhrasingContent[] }
export interface HeadingNode { type: 'heading'; depth: 1 | 2 | 3 | 4 | 5 | 6; children: PhrasingContent[] }
export interface BlockquoteNode { type: 'blockquote'; children: FlowContent[] }
export interface ListNode { type: 'list'; ordered?: boolean; start?: number; children: ListItemNode[] }
export interface ListItemNode { type: 'listItem'; checked?: boolean; children: FlowContent[] }
export interface CodeNode { type: 'code'; lang?: string; value: string }
export interface TableNode { type: 'table'; align?: Array<'left' | 'center' | 'right' | null>; children: TableRowNode[] }
export interface TableRowNode { type: 'tableRow'; children: TableCellNode[] }
export interface TableCellNode { type: 'tableCell'; children: PhrasingContent[] }
export interface ThematicBreakNode { type: 'thematicBreak' }
export interface HtmlNode { type: 'html'; value: string }
export interface MathBlockNode { type: 'mathBlock'; value: string; display: true }
export interface MermaidNode { type: 'mermaid'; value: string }
...
export const PhrasingContentSchema: z.ZodType<PhrasingContent> = z.lazy(() =>
  z.discriminatedUnion('type', [
    ...
    z.object({ type: z.literal('link'), url: z.string(), title: z.string().optional(), children: z.array(z.any()) }),
    z.object({ type: z.literal('image'), url: z.string(), alt: z.string().optional(), title: z.string().optional() }),
...
export const FlowContentSchema: z.ZodType<FlowContent> = z.lazy(() =>
  z.discriminatedUnion('type', [
    ...
    z.object({ type: z.literal('heading'), depth: z.union([z.literal(1), z.literal(2), z.literal(3), z.literal(4), z.literal(5), z.literal(6)]), children: z.array(z.any()) }),
    z.object({ type: z.literal('blockquote'), children: z.array(z.any()) }),
    z.object({ type: z.literal('list'), ordered: z.boolean().optional(), start: z.number().optional(), children: z.array(z.any()) }),
    z.object({ type: z.literal('listItem'), checked: z.boolean().optional(), children: z.array(z.any()) }),
    z.object({ type: z.literal('code'), lang: z.string().optional(), value: z.string() }),
    z.object({ type: z.literal('table'), align: z.array(z.union([z.literal('left'), z.literal('center'), z.literal('right'), z.null()])).optional(), children: z.array(z.any()) }),
...
export const RichNodeSchema: z.ZodType<RichNode> = z.union([FlowContentSchema, PhrasingContentSchema]) as z.ZodType<RichNode>
...
export function parseRichText(md: string): RichNode[] {
  if (!md) return []
  md = stripCodeFenceDelimiters(md)
...
function stripCodeFenceDelimiters(md: string): string {
  return md.replace(/^```[\s\S]*?```$/gm, (match) => {
...
const FLOW_TYPES = new Set<string>(['paragraph','heading','blockquote','list','listItem','code','table','thematicBreak','html','mathBlock','mermaid'])
function isFlowNode(type: string): boolean { return FLOW_TYPES.has(type) }
...
function convertMdastNode(n: MdastNode): RichNode | null {
  switch (n.type) {
    ...
    case 'heading':
      return { type: 'heading', depth: (n.depth ?? 1) as 1 | 2 | 3 | 4 | 5 | 6, children: normalizePhrasing(n.children ?? []) }
    case 'blockquote':
      return { type: 'blockquote', children: normalizeTree(n.children ?? []) }
    case 'list':
      return { type: 'list', ordered: n.ordered, start: n.start, children: normalizeListItems(n.children ?? []) }
    case 'listItem':
      return { type: 'listItem', checked: n.checked ?? undefined, children: normalizeTree(n.children ?? []) }
    case 'code':
    ...
    case 'link':
      return { type: 'link', url: n.url ?? '', title: n.title, children: normalizePhrasing(n.children ?? []) }
    case 'image':
...
function normalizePhrasing(nodes: MdastNode[]): PhrasingContent[] {
  return nodes.map(convertMdastNode).filter((n): n is PhrasingContent => n !== null && !('paragraph' in n))
}
...
function serializePhrasing(nodes: PhrasingContent[]): string {
  return nodes.map((n) => {
    switch (n.type) {
      ...
      case 'emphasis': return '*' + serializePhrasing(n.children) + '*'
      case 'strong': return '**' + serializePhrasing(n.children) + '**'
      case 'delete': return '~~' + serializePhrasing(n.children) + '~~'
      case 'inlineCode': return '`' + n.value + '`'
      case 'link': return '[' + serializePhrasing(n.children) + '](' + n.url + (n.title ? ' "' + n.title + '"' : '') + ')'
      case 'image': return '![' + (n.alt ?? '') + '](' + n.url + (n.title ? ' "' + n.title + '"' : '') + ')'
      case 'break': return '\n'
      case 'math': return '$' + n.value + '$'
      case 'widget': return `<widget:${n.kind}>`
      case 'mention': return '@' + n.id
      default: return ''
    }
  }).join('')
}
...
function serializeTable(table: TableNode): string {
  ...
  const headerDivider = '| ' + align.map((a) => {
    switch (a) {
      case 'left': return ':---'
      case 'center': return ':--:'
      case 'right': return '---:'
