...
import type { AIError, AIErrorCode, ModelId, ProviderId } from './types.js'
...
export const AI_ERRORS = {
  ...
  policyDenied(reason: string): VivimAIError {
    return new VivimAIError({
      code: 'POLICY_DENIED',
      message: `Execution blocked by policy: ${reason}`,
      retryable: false,
    })
  },
  toolDenied(toolName: string, reason: string): VivimAIError {
    return new VivimAIError({
      code: 'TOOL_DENIED',
      message: `Tool "${toolName}" denied: ${reason}`,
      retryable: false,
    })
  },
  ...
  runtimeCrash(providerId: ProviderId, cause?: unknown): VivimAIError {
    return new VivimAIError(
      {
        code: 'RUNTIME_CRASH',
        message: `Provider ${providerId} runtime crashed.`,
        retryable: true,
        providerId,
      },
      { cause },
  ...
  pluginInvalid(pluginName: string, reason: string): VivimAIError {
    return new VivimAIError({
      code: 'PLUGIN_INVALID',
      message: `Plugin "${pluginName}" is invalid: ${reason}`,
      retryable: false,
    })
  },
