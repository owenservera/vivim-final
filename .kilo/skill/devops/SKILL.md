...
```
LOOP:
  0. CONTEXT CHECK: Run `bun run devops context` to detect:
     - Active plan files in `.kilo/plans/` (most recently modified)
     - Session objectives at `docs/session/current.md`
     - Current focus unit (units marked `in_progress`)
     If context exists and context != atomic loop → report context and ask
     "Continue context or switch to atomic loop?"
...
| Command | Purpose |
|---------|---------|
| `bun run devops context` | Print current context (active plan, session objectives, focus unit) |
