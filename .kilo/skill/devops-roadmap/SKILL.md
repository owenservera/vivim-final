...
name: devops-roadmap
description: >
  Research-first roadmap system for vivim-final. Enriches the atomic tracker
  with truth-grounded research data, discovers new units from gaps, and
  facilitates human-AI interview to refine discovered units into atomic specs.
  Use BEFORE the devops loop to understand what already exists, what needs
  porting, and what's genuinely missing. Triggers: "devops roadmap",
  "research first", "audit before implementing", "what already exists",
  "gap analysis", "discover new units".
---
...
2. Read existing code (if file exists)
3. Read existing vivim-final code — the file is the source of truth (if it exists)
4. Compare: spec vs existing vivim-final code
