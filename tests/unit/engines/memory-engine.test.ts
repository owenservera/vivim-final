...
import { MemoryEngine, type EpisodicMemoryStore, type SemanticMemoryStore, type ProceduralMemoryStore } from '../../../src/engines/memory-engine.js'
...
describe('MemoryEngine', () => {
  it('uses ULID IDs for episodes', async () => {
    ...
    stores.episodic.save = async (ep) => { captured.push(ep.id) }
    const engine = new MemoryEngine(stores.episodic, stores.semantic, stores.procedural, eventBusAccessor())
    await engine.recordEpisode({ providerId: 'p1', action: 'test', input: {}, output: {}, success: true, durationMs: 100 })
  ...
  it('recordDecision emits event', async () => {
    ...
    await engine.recordDecision({
      conversationId: 'c1', messageId: 'm1', decisionText: 'Use TypeScript',
    })
  })
