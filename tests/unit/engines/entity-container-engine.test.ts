...
function makeStore() {
  ...
  const store: EntityContainerStore = {
    ...
    getContainersByType: vi.fn(async (t) =>
      [...containers.values()].filter((c) => c.containerType === t),
    ),
    getContainersByProvider: vi.fn(async (p, a) =>
      [...containers.values()].filter((c) => c.providerId === p && c.accountId === a),
    ),
    ...
    addMembership: vi.fn(async (input: ContainerMembershipInput): Promise<ContainerMembership> => {
      const m: ContainerMembership = {
        id: `m-${memberships.size + 1}`,
        userRole: input.userRole ?? 'member',
        joinedAt: 1,
        isFavorite: 0,
    ...
    removeMembership: vi.fn(async (containerId, userRole) => {
      ...
      memberships.set(
        containerId,
        list.filter((m) => m.userRole !== userRole),
      )
    }),
  }
  return { store, bus }
...
describe('EntityContainerEngine', () => {
  test('createContainer emits', async () => {
    ...
    const c = await engine.createContainer({
      providerId: 'p',
      accountId: 'a',
