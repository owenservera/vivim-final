...
import { MediaEngine, type MediaAttachment, type MediaAttachmentInput, type MediaStore } from '../../../src/engines/media-engine.js'
...
function makeStore() {
  ...
  const store: MediaStore = {
    createMedia: vi.fn(async (input: MediaAttachmentInput): Promise<MediaAttachment> => {
      const m: MediaAttachment = { id: `m-${media.size + 1}`, isDownloaded: 0, isEncrypted: 0, downloadProgress: 0, metadataJson: '{}', createdAt: 1, updatedAt: 1, ...input }
      media.set(m.id, m)
      return m
    }),
    getMediaById: vi.fn(async (id) => media.get(id) ?? null),
    getMediaByContentItem: vi.fn(async (cid) => [...media.values()].filter((m) => m.contentItemId === cid)),
...
describe('MediaEngine', () => {
  test('createMedia emits', async () => {
    ...
    const m = await engine.createMedia({ providerId: 'p', mediaType: 'image', mimeType: 'image/png', originalUrl: 'http://x/y.png' })
  ...
  test('listByContentItem filters', async () => {
    const { store } = makeStore()
    const engine = new MediaEngine(store)
    await engine.createMedia({ providerId: 'p', contentItemId: 'c1', mediaType: 'image', mimeType: 'image/png', originalUrl: 'a' })
    await engine.createMedia({ providerId: 'p', contentItemId: 'c2', mediaType: 'image', mimeType: 'image/png', originalUrl: 'b' })
  ...
  test('listByType filters', async () => {
    const { store } = makeStore()
    const engine = new MediaEngine(store)
    await engine.createMedia({ providerId: 'p', mediaType: 'video', mimeType: 'video/mp4', originalUrl: 'a' })
  ...
  test('getUndownloaded returns only undownloaded', async () => {
    const { store } = makeStore()
    const engine = new MediaEngine(store)
    await engine.createMedia({ providerId: 'p', mediaType: 'image', mimeType: 'image/png', originalUrl: 'a' })
  ...
  test('markDownloaded emits', async () => {
    const { store, bus } = makeStore()
    const engine = new MediaEngine(store, bus as never)
    const m = await engine.createMedia({ providerId: 'p', mediaType: 'image', mimeType: 'image/png', originalUrl: 'a' })
  ...
  test('deleteMedia emits', async () => {
    ...
    expect(bus.emit).toHaveBeenCalledWith(expect.objectContaining({ type: 'media:deleted', mediaId: m.id }))
  })
})
