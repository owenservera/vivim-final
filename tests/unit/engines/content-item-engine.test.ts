...
import { ContentItemEngine, type ContentItem, type ContentItemInput, type ContentItemStore } from '../../../src/engines/content-item-engine.js'
...
function makeStore() {
  ...
  const store: ContentItemStore = {
    createItem: vi.fn(async (input: ContentItemInput): Promise<ContentItem> => {
      const it: ContentItem = { id: `i-${items.size + 1}`, isEdited: 0, isPinned: 0, isDeleted: 0, isBookmarked: 0, replyCount: 0, shareCount: 0, sequenceIndex: 0, sortTimestamp: 1, bodyRichJson: '{}', mediaAttachmentsJson: '[]', reactionsJson: '[]', tagsJson: '[]', mentionsJson: '[]', linksJson: '[]', editHistoryJson: '[]', metadataJson: '{}', createdAt: 1, updatedAt: 1, ...input }
      items.set(it.id, it)
      return it
    }),
    getItemById: vi.fn(async (id) => items.get(id) ?? null),
    getItemByExternalId: vi.fn(async (p, n) => [...items.values()].find((i) => i.providerId === p && i.providerNativeId === n) ?? null),
    queryItems: vi.fn(async (q) => [...items.values()].filter((i) => !q.containerId || i.containerId === q.containerId)),
...
describe('ContentItemEngine', () => {
  test('createItem delegates and emits', async () => {
    ...
    const it = await engine.createItem({ providerId: 'p', accountId: 'a', contentType: 'post', bodyText: 'hello' })
  ...
  test('getByExternalId resolves by provider native id', async () => {
    const { store } = makeStore()
    const engine = new ContentItemEngine(store)
    const it = await engine.createItem({ providerId: 'p', accountId: 'a', providerNativeId: 'ext1', contentType: 'post' })
  ...
  test('queryItems filters by containerId', async () => {
    const { store } = makeStore()
    const engine = new ContentItemEngine(store)
    await engine.createItem({ providerId: 'p', accountId: 'a', containerId: 'c1', contentType: 'post' })
    await engine.createItem({ providerId: 'p', accountId: 'a', containerId: 'c2', contentType: 'post' })
  ...
  test('deleteItem emits', async () => {
    ...
    expect(bus.emit).toHaveBeenCalledWith(expect.objectContaining({ type: 'content:deleted', itemId: it.id }))
  })

  test('searchItems delegates', async () => {
    const { store } = makeStore()
    const engine = new ContentItemEngine(store)
    await engine.createItem({ providerId: 'p', accountId: 'a', contentType: 'post', bodyText: 'needle here' })
