...
import { ContactEngine, type Contact, type ContactInput, type ContactStore } from '../../../src/engines/contact-engine.js'
...
function makeStore() {
  ...
  const store: ContactStore = {
    createContact: vi.fn(async (input: ContactInput): Promise<Contact> => {
      const c: Contact = { id: `c-${contacts.size + 1}`, isOnline: 0, isFavorite: 0, isBlocked: 0, relationship: 'other', metadataJson: '{}', createdAt: 1, updatedAt: 1, ...input }
      contacts.set(c.id, c)
      return c
    }),
    getContactById: vi.fn(async (id: string) => contacts.get(id) ?? null),
    getContactByNativeId: vi.fn(async (p, a, n) => [...contacts.values()].find((c) => c.providerId === p && c.accountId === a && c.providerNativeId === n) ?? null),
    getContactsByAccount: vi.fn(async (a) => [...contacts.values()].filter((c) => c.accountId === a)),
    searchContacts: vi.fn(async (q, a) => [...contacts.values()].filter((c) => (!a || c.accountId === a) && c.displayName.includes(q))),
    ...
    mergeContacts: vi.fn(async (canonicalId, mergedId, method, confidence) => {
      ...
      const row = { id, canonicalContactId: canonicalId, mergedContactId: mergedId, mergeConfidence: confidence, mergeMethod: method, isConfirmed: 0, createdAt: 1 }
...
describe('ContactEngine', () => {
  test('createContact delegates and emits event', async () => {
    ...
    const c = await engine.createContact({ providerId: 'p', accountId: 'a', providerNativeId: 'n', displayName: 'Sam' })
  ...
  test('listContacts filters by account', async () => {
    const { store } = makeStore()
    const engine = new ContactEngine(store)
    await engine.createContact({ providerId: 'p', accountId: 'a1', providerNativeId: 'n1', displayName: 'A' })
    await engine.createContact({ providerId: 'p', accountId: 'a2', providerNativeId: 'n2', displayName: 'B' })
  ...
  test('searchContacts delegates with query', async () => {
    const { store } = makeStore()
    const engine = new ContactEngine(store)
    await engine.createContact({ providerId: 'p', accountId: 'a', providerNativeId: 'n1', displayName: 'Sam' })
  ...
  test('deleteContact emits event', async () => {
    ...
    expect(bus.emit).toHaveBeenCalledWith(expect.objectContaining({ type: 'contact:deleted', contactId: c.id }))
  })

  test('mergeContacts and getMergedContacts', async () => {
    const { store } = makeStore()
    const engine = new ContactEngine(store)
    const a = await engine.createContact({ providerId: 'p', accountId: 'a', providerNativeId: 'n1', displayName: 'A' })
    const b = await engine.createContact({ providerId: 'p', accountId: 'a', providerNativeId: 'n2', displayName: 'B' })
