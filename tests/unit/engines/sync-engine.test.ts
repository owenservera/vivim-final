...
import { SyncEngine, type SyncState, type SyncStateInput, type SyncStateStore } from '../../../src/engines/sync-engine.js'
...
function makeStore() {
  ...
  const store: SyncStateStore = {
    upsertSyncState: vi.fn(async (input: SyncStateInput): Promise<SyncState> => {
      const s: SyncState = { id: `s-${states.size + 1}`, syncDirection: 'pull', syncStatus: 'pending', syncVersion: 1, cursorJson: '{}', errorCount: 0, itemsSynced: 0, itemsFailed: 0, bytesSynced: 0, metadataJson: '{}', createdAt: 1, updatedAt: 1, ...input }
      states.set(s.id, s)
      return s
    }),
    getSyncState: vi.fn(async (p, a, t, e) => [...states.values()].find((s) => s.providerId === p && s.accountId === a && s.entityType === t && s.entityId === e) ?? null),
    getSyncStatesByAccount: vi.fn(async (a) => [...states.values()].filter((s) => s.accountId === a)),
    getSyncStatesPending: vi.fn(async () => [...states.values()].filter((s) => s.syncStatus === 'pending')),
    ...
    incrementSyncStats: vi.fn(async (id, synced, failed, bytes) => {
      const cur = states.get(id)!
      const next = { ...cur, itemsSynced: cur.itemsSynced + synced, itemsFailed: cur.itemsFailed + failed, bytesSynced: cur.bytesSynced + bytes }
      states.set(id, next)
      return next
    }),
...
describe('SyncEngine', () => {
  test('upsertSyncState emits', async () => {
    ...
    const s = await engine.upsertSyncState({ providerId: 'p', accountId: 'a', entityType: 'contact', entityId: 'e1' })
  ...
  test('listSyncStates by account', async () => {
    const { store } = makeStore()
    const engine = new SyncEngine(store)
    await engine.upsertSyncState({ providerId: 'p', accountId: 'a1', entityType: 'contact', entityId: 'e1' })
    await engine.upsertSyncState({ providerId: 'p', accountId: 'a2', entityType: 'contact', entityId: 'e2' })
  ...
  test('getPendingSyncs returns pending only', async () => {
    ...
    await engine.upsertSyncState({ providerId: 'p', accountId: 'a', entityType: 'contact', entityId: 'e2' })
  ...
  test('deleteSyncState emits', async () => {
    ...
    expect(bus.emit).toHaveBeenCalledWith(expect.objectContaining({ type: 'sync:deleted', stateId: s.id }))
  })
})
