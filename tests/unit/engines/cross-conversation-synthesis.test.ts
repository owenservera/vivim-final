...
import { CrossConversationSynthesizer, type SynthesisLlmProvider } from '../../../src/engines/cross-conversation-synthesis.js'
...
function mockSemanticStore(): SemanticSearchStore {
  ...
  return {
    upsertEmbedding: async (input) => { embeddings.set(`${input.entityType}:${input.entityId}`, input) },
    getEmbedding: async (et, eid) => { const e = embeddings.get(`${et}:${eid}`); return e ? { id: e.id, embedding: e.embedding, model: e.model, dimensions: e.dimensions } : null },
    searchByEmbedding: async (q, opts) => {
      ...
      for (const e of embeddings.values()) {
        ...
        for (let i = 0; i < q.length; i++) {
          dot += q[i]! * stored[i]!; mA += q[i]! ** 2; mB += stored[i]! ** 2
        }
        const score = dot / (Math.sqrt(mA) * Math.sqrt(mB))
        if (score >= (opts.threshold ?? 0)) results.push({ entityId: e.entityId, entityType: e.entityType, score })
      }
...
function mockSynthesisStore(): CrossConversationSynthesizerStore {
  return {
    ...
    getDecisionsForConversation: async () => [
      { id: 'd1', decisionText: 'Use TypeScript for the backend', rationale: 'type safety', confidence: 0.8 },
    ],
...
describe('CrossConversationSynthesizer', () => {
  it('synthesizes answer from relevant conversations', async () => {
    ...
    const llm: SynthesisLlmProvider = {
      synthesize: async () => ({ text: 'TypeScript is a typed superset of JavaScript', confidence: 0.85 }),
    }
