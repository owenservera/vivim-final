...
describe('zodToJsonSchema', () => {
  ...
  test('array of strings', () => {
    const json = zodToJsonSchema(z.object({ files: z.array(z.string()) }))
  ...
  test('defaulted field is not required', () => {
    const json = zodToJsonSchema(z.object({ timeoutMs: z.number().default(5000) }))
    expect(json.properties?.timeoutMs).toEqual({ type: 'number', default: 5000 })
    expect(json.required).toEqual([])
  })

  test('record<string, string> passthrough', () => {
    const json = zodToJsonSchema(z.object({ headers: z.record(z.string()) }))
