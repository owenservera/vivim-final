...
import type { MockTable } from '../../helpers/prisma-mock.js'
...
describe('KnowledgeIngestionStoreImpl', () => {
  ...
  it('createImportJob + getImportJob round-trips', async () => {
    await store.createImportJob({
      id: 'job_1', source: 'chatgpt', filePath: '/tmp/export.json',
      status: 'pending', configJson: '{}', startedAt: 1000,
    })
  ...
  it('updateImportJob patches status', async () => {
    await store.createImportJob({
      id: 'job_2', source: 'claude', filePath: '/tmp/export.json',
      status: 'pending', configJson: '{}', startedAt: 1000,
    })
  ...
  it('listImportJobs returns jobs ordered by startedAt desc', async () => {
    await store.createImportJob({
      id: 'a', source: 's1', filePath: '/a.json', status: 'done',
      configJson: '{}', startedAt: 100,
    })
    await store.createImportJob({
      id: 'b', source: 's2', filePath: '/b.json', status: 'pending',
      configJson: '{}', startedAt: 200,
    })
...
describe('KnowledgeExtractorStoreImpl', () => {
  ...
  it('createEntity + findEntityByName round-trips', async () => {
    await store.createEntity({
      id: 'ent_1', name: 'PostgreSQL', type: 'technology',
      description: 'Database', confidence: 0.9, firstSeenAt: 100, lastSeenAt: 200,
    })
  ...
  it('updateEntity patches confidence', async () => {
    await store.createEntity({
      id: 'ent_2', name: 'Redis', type: 'technology',
      description: null, confidence: 0.5, firstSeenAt: 100, lastSeenAt: 100,
    })
  ...
  it('createEntityMention stores record', async () => {
    await store.createEntity({
      id: 'ent_3', name: 'React', type: 'framework',
      description: null, confidence: 0.8, firstSeenAt: 100, lastSeenAt: 100,
    })
    await store.createEntityMention({
      id: 'em_1', entityId: 'ent_3', conversationId: 'conv_1',
      messageId: 'msg_1', context: 'using React', confidence: 0.8, ts: 100,
    })
  ...
  it('createDecision stores record', async () => {
    await store.createDecision({
      id: 'dec_1', conversationId: 'conv_1', messageId: 'msg_1',
      decisionText: 'Use PostgreSQL', rationale: 'Best for relational',
      alternatives: '[]', confidence: 0.9, ts: 100,
    })
  ...
  it('createPattern + findPattern round-trips', async () => {
    await store.createPattern({
      id: 'pat_1', name: 'DB choice', description: 'Team picks DB',
      patternType: 'decision', occurrences: 1, confidence: 0.7,
  ...
  it('updatePattern patches occurrences', async () => {
    await store.createPattern({
      id: 'pat_2', name: 'API design', description: 'REST vs GraphQL',
      patternType: 'decision', occurrences: 1, confidence: 0.6,
      firstSeenAt: 100, lastSeenAt: 100,
    })
  ...
  it('assertSemanticMemory creates record', async () => {
    await store.assertSemanticMemory({
      id: 'sm_1', subject: 'vivim', predicate: 'uses',
      objectJson: '"PostgreSQL"', confidence: 0.95, source: 'conv_1',
      timestamp: 100, expiresAt: null,
    })
...
describe('SemanticSearchStoreImpl', () => {
  ...
  it('upsertEmbedding creates new embedding', async () => {
    await store.upsertEmbedding({
      id: 'emb_1', entityType: 'fact', entityId: 'fact_1',
      embedding: '[0.1,0.2,0.3]', model: 'text-embedding-3-small',
      dimensions: 3, contentHash: 'abc', createdAt: 100,
    })
  ...
  it('getEmbedding retrieves by type + id', async () => {
    await store.upsertEmbedding({
      id: 'emb_2', entityType: 'entity', entityId: 'ent_1',
      embedding: '[0.4,0.5]', model: 'test', dimensions: 2,
      contentHash: 'def', createdAt: 100,
    })
  ...
  it('searchByEmbedding returns scored results', async () => {
    ...
    db.prisma.memoryEmbedding.rows.push(
      { id: 'emb_a', entityType: 'fact', entityId: 'f_a', embedding: '[1,0,0]', model: 'test', dimensions: 3, contentHash: 'a', createdAt: 100 },
      { id: 'emb_b', entityType: 'fact', entityId: 'f_b', embedding: '[0,1,0]', model: 'test', dimensions: 3, contentHash: 'b', createdAt: 100 },
    )
  ...
  it('searchByEmbedding filters by threshold', async () => {
    db.prisma.memoryEmbedding.rows.push(
      { id: 'emb_c', entityType: 'fact', entityId: 'f_c', embedding: '[1,0]', model: 'test', dimensions: 2, contentHash: 'c', createdAt: 100 },
      { id: 'emb_d', entityType: 'fact', entityId: 'f_d', embedding: '[0,1]', model: 'test', dimensions: 2, contentHash: 'd', createdAt: 100 },
    )
  ...
  it('deleteEmbedding removes entry', async () => {
    await store.upsertEmbedding({
      id: 'emb_e', entityType: 'fact', entityId: 'f_e',
      embedding: '[1]', model: 'test', dimensions: 1,
      contentHash: 'e', createdAt: 100,
    })
  ...
  it('countEmbeddings returns count', async () => {
    ...
    db.prisma.memoryEmbedding.rows.push(
      { id: 'emb_f', entityType: 'fact', entityId: 'f_f', embedding: '[1]', model: 'test', dimensions: 1, contentHash: 'f', createdAt: 100 },
      { id: 'emb_g', entityType: 'entity', entityId: 'e_g', embedding: '[1]', model: 'test', dimensions: 1, contentHash: 'g', createdAt: 100 },
    )
...
describe('CrossConversationSynthesizerStoreImpl', () => {
  ...
  it('getFactsForConversation returns facts by source', async () => {
    await db.prisma.semanticMemory.create({
      data: {
        id: 'sm_1', subject: 'vivim', predicate: 'uses', object_json: '"PostgreSQL"',
        confidence: 0.9, source: 'conv_1', timestamp: 100, expires_at: null,
      },
    })
    await db.prisma.semanticMemory.create({
      data: {
        id: 'sm_2', subject: 'vivim', predicate: 'runs on', object_json: '"Bun"',
        confidence: 0.8, source: 'conv_1', timestamp: 200, expires_at: null,
      },
    })
    await db.prisma.semanticMemory.create({
      data: {
        id: 'sm_3', subject: 'other', predicate: 'is', object_json: '"different"',
        confidence: 0.5, source: 'conv_2', timestamp: 300, expires_at: null,
      },
    })
  ...
  it('getDecisionsForConversation returns decisions', async () => {
    await db.prisma.decisionRecord.create({
      data: {
        id: 'dec_1', conversationId: 'conv_1', messageId: 'msg_1',
        decisionText: 'Use TypeScript', rationale: 'Type safety',
        alternativesJson: '[]', confidence: 0.9, ts: 100,
      },
    })
  ...
  it('getEntitiesForConversation returns entities from mentions', async () => {
    ...
    db.prisma.entityMention.rows.push({
      id: 'em_1', entityId: 'ent_1', conversationId: 'conv_1',
      messageId: 'msg_1', context: 'using React', confidence: 0.85, ts: 100,
