...
export class ProviderMuxEngine {
  ...
  async fanOut(message: string, providerIds: string[], requestOrTimeout: MuxRequest | number): Promise<MuxResponse> {
    const timeoutMs = typeof requestOrTimeout === 'number' ? requestOrTimeout : requestOrTimeout.timeoutMs
    const request = typeof requestOrTimeout === 'object' ? requestOrTimeout : {
      timeoutMs, synthesisEnabled: true, maxProviders: providerIds.length, strategy: 'fan_out' as MuxStrategy,
    }
  ...
  async autoRoute(message: string, capabilityId?: string): Promise<MuxResponse> {
    ...
    if (providerIds.length === 0) {
      throw new Error(`No routing preferences found${capabilityId ? ` for capability ${capabilityId}` : ''}`)
    }
  ...
  private async roundRobin(message: string, providerIds: string[], request: MuxRequest): Promise<MuxResponse> {
    ...
    for (const pid of providerIds.slice(0, request.maxProviders)) {
      const result = await this.dispatchWithTimeout(pid, message, sessionId, request.timeoutMs, request.conversationId)
      results.push(result)
    }

    return this.buildResponse(sessionId, providerIds, results, 'round_robin', request.synthesisEnabled)
  }

  private async priorityDispatch(message: string, providerIds: string[], request: MuxRequest): Promise<MuxResponse> {
  ...
  private async costOptimized(message: string, providerIds: string[], request: MuxRequest): Promise<MuxResponse> {
    ...
    return this.buildResponse(sessionId, providerIds, results, 'cost_optimized', request.synthesisEnabled)
  }
  ...
  private async createSession(message: string, conversationId: string | null, strategy: string): Promise<string> {
    const id = newId()
  ...
  private async buildResponse(
    ...
    const best = providerResponses.filter((r) => r.ok).sort((a, b) => a.latencyMs - b.latencyMs)[0] ?? null
