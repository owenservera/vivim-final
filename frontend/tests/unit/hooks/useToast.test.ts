...
describe('useToast', () => {
  ...
  test('showToast sets toast state', () => {
    ...
    act(() => { result.current.showToast('ok', 'Saved!'); });
  ...
  test('showToast with error kind', () => {
    ...
    act(() => { result.current.showToast('err', 'Failed!'); });
  ...
  test('auto-dismisses after specified time', () => {
    ...
    act(() => { result.current.showToast('ok', 'Temp'); });
  ...
  test('clearToast clears immediately', () => {
    ...
    act(() => { result.current.showToast('ok', 'Clear me'); });
    expect(result.current.toast).not.toBeNull();
  ...
  test('new toast replaces old one', () => {
    ...
    act(() => { result.current.showToast('ok', 'First'); });
    expect(result.current.toast?.msg).toBe('First');

    act(() => { result.current.showToast('err', 'Second'); });
