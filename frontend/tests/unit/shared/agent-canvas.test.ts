...
describe('agent-canvas types', () => {
  describe('AgentCanvasCommand', () => {
      ...
      const commands: AgentCanvasCommand[] = [
        {
          ...
          payload: { slotId: 'chat.thread', providerId: 'chatgpt', position: { x: 100, y: 100 } },
        },
        ...
        { type: 'canvas.runLayout', payload: { intent: 'custom', params: { foo: 'bar' } } },
        {
          ...
          payload: { nodeId: 'node:1', capabilityId: 'send_message', input: { text: 'hello' } },
        },
        ...
        {
          type: 'canvas.getState',
          payload: { includeConnections: true, includeNodes: ['node:1'] },
        },
      ]
    ...
    it('should infer correct payload types per command', () => {
      const createNode: AgentCanvasCommand = {
        type: 'canvas.createNode',
      ...
      const runLayout: AgentCanvasCommand = {
        type: 'canvas.runLayout',
        payload: { intent: 'timeline', params: { direction: 'horizontal' } },
      }
  ...
  describe('AgentCanvasResponse', () => {
    it('should accept all valid response types', () => {
      const responses: AgentCanvasResponse[] = [
        {
          type: 'canvas.state',
          payload: { nodes: [], connections: [], viewport: { x: 0, y: 0, zoom: 1 }, streams: [] },
        },
        {
          type: 'canvas.nodeCreated',
          payload: { nodeId: 'node:1', slotId: 'chat.thread', providerId: 'chatgpt' },
        },
    ...
    it('should accept all valid op types', () => {
      const ops: AgentCanvasOp[] = [
        ...
        },
        {
          id: 'op:3',
          type: 'moveNode',
          payload: { nodeId: 'node:1', x: 100, y: 100 },
          status: 'pending',
          createdAt: Date.now(),
        },
        {
          id: 'op:4',
          type: 'connectNodes',
          payload: { fromNodeId: 'node:1', toNodeId: 'node:2' },
          status: 'pending',
          createdAt: Date.now(),
        },
        {
        ...
        },
        {
          id: 'op:6',
          type: 'startStream',
          payload: { nodeId: 'node:1', capabilityId: 'send_message' },
          status: 'pending',
          createdAt: Date.now(),
        },
        {
