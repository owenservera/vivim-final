...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  const resp = await fetch(path, { ...init, headers, signal: init?.signal ?? AbortSignal.timeout(15000) })
...
export const capabilityApi = {
  listBySurface: (surface: string) => request<ResolvedCapabilityDto[]>(`/api/capabilities?surface=${surface}`),
  execute: (capabilityId: string, input: Record<string, unknown>) =>
    request<{ ok: boolean; result?: unknown; error?: string }>(`/api/capabilities/${encodeURIComponent(capabilityId)}/execute`, {
      method: 'POST',
