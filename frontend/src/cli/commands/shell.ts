...
export function registerDefaultCommands(store: ShellCommandStore): void {
  const commands: CommandSpec[] = [
    ...
    },
    {
      ...
      handler: async (_args, ctx) =>
        ok(ctx, 'Reset aborted: --force flag required for destructive ops.'),
    },
    {
    ...
    // ── publish / patch (live-config) ──────────────────────────────────
    {
      ...
      handler: async (args, ctx) =>
        ok(
          ctx,
          `Published component: ${args.join(' ') || '(no slug)'}\n  → version=1, emitted canvas:def:updated`,
        ),
    },
    {
  ...
  commands.push({
    path: ['go'],
    description:
      'Switch surface (chat/docs/media/automation/agents/shell/audit/rbac/templates/zlayers/editor)',
  ...
  commands.push({
    ...
    handler: async (args, ctx) =>
      ok(ctx, `→ Layout: ${args[0] ?? 'free'} (force-directed re-layout triggered)`),
  })
  commands.push({
    path: ['canvas', 'zoom'],
  ...
  commands.push({
    ...
    handler: async (args, ctx) =>
      ok(ctx, `→ Streaming node ${args[0] ?? '?'} via ${args[1] ?? 'cap:default'}`),
  })
  ...
  commands.push({
    ...
    handler: async (args, ctx) =>
      ok(ctx, `→ Connected ${args[0] ?? '?'} → ${args[1] ?? '?'} (${args[2] ?? 'data'})`),
  })
  ...
  commands.push({
    ...
    handler: async (args, ctx) =>
      ok(ctx, `→ Drawer ${args[0] ?? 'left'} → panel ${args[1] ?? 'conversations'}`),
  })
  ...
  commands.push({
    ...
    handler: async (args, ctx) =>
      ok(ctx, `→ Z-Layer ${args[1] ?? 'content'}: ${args[0] ?? 'show'}`),
  })
      ...
      try {
        ...
        for (const [kind, comps] of Object.entries(byKind)) {
          ...
          for (const c of comps) {
            lines.push(
              `    ${c.id.padEnd(28)} ${c.label.padEnd(24)} [${c.category}] ${c.capabilities.length} caps`,
            )
          }
        }
    ...
    handler: async (_args, ctx) => {
      try {
        ...
        for (const c of all) {
          ...
          lines.push(
            `  ${c.id.padEnd(28)} ${c.label.padEnd(24)} ${c.kind.padEnd(10)} ${c.category.padEnd(12)} ${caps} caps${tags}`,
          )
        }
        lines.push(`\n  Total: ${all.length} components`)
        return ok(ctx, lines.join('\n'))
  ...
  commands.push({
    path: ['ui', 'extend'],
    description:
      'Extend a base component spec (creates a derived component with merged properties)',
    ...
    handler: async (args, ctx) => {
      ...
      try {
        ...
        const data = (await res.json()) as {
          ok: boolean
          spec?: { id: string; label: string; version: number }
          error?: string
        }
        if (!data.ok) return ok(ctx, `Error: ${data.error}`)
        return ok(
          ctx,
          `→ Extended ${baseId} → ${data.spec?.id} (${data.spec?.label}) v${data.spec?.version}`,
        )
      } catch (err) {
        return ok(ctx, `Error: ${String(err)}`)
      }
