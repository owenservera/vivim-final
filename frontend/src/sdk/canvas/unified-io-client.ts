...
export function createUnifiedIO(
  opts: { fetchImpl?: typeof fetch; ulidImpl?: () => string; apiBase?: string } = {},
): UnifiedIO {
  ...
  return {
        ...
        try {
          ...
          const ioError =
            err instanceof IOError
              ? err
              : new IOError(isAbort ? 'Request timed out' : String(err), 0, traceId, err)
          emit({
            type: 'request:error',
            traceId,
            method,
    ...
    subscribeSSE(
      ...
      const traceId = ulid()
      const Es = (typeof EventSource !== 'undefined' ? EventSource : undefined) as
        | typeof EventSource
        | undefined
      if (!Es) {
