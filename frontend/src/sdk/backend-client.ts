...
export type CapabilityList = { capabilities: z.infer<typeof CapabilitySchema>[]; total?: number }

const CapabilityListSchema = z.union([
...
export async function listCapabilities(surface?: string) {
  ...
  const res = await request(`/api/capabilities${qs}`, CapabilityListSchema)
  if (!res.ok) return res as BackendResponse<CapabilityList>
  const raw = res.data!
  const capabilities = Array.isArray(raw) ? raw : raw.capabilities
  return { ok: res.ok, data: { capabilities, total: capabilities.length } as CapabilityList, status: res.status }
}
...
export async function getProviderCapabilities(providerId: string, planTier?: string) {
  ...
  const res = await request(`/api/providers/${encodeURIComponent(providerId)}/capabilities${qs}`, CapabilityListSchema)
  if (!res.ok) return res as BackendResponse<CapabilityList>
  const raw = res.data!
  const capabilities = Array.isArray(raw) ? raw : raw.capabilities
  return { ok: res.ok, data: { capabilities, total: capabilities.length } as CapabilityList, status: res.status }
...
export async function ingestKnowledge(filePath: string, source?: string) {
  return request("/api/knowledge/ingest", z.object({ id: z.string() }), {
    method: "POST",
    body: JSON.stringify({ source: source ?? "frontend", filePath }),
  })
}
...
/** End session — DELETE /api/fleet/:id (kills Chrome slave). */
export async function endSession(sessionId: string) {
  return request(`/api/fleet/${encodeURIComponent(sessionId)}`, z.object({ ok: z.boolean() }), {
    method: "DELETE",
  })
}
