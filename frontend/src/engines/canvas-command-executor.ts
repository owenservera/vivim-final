...
import { getCanvasEventBus, CanvasEventType, type CanvasEvent } from '@/components/canvas/event-bus';

export class CanvasCommandExecutor {
  private policy: AgentCanvasPolicy;
  private workspaceId: string;
  private agentId: string;
  private pendingConfirmations = new Map<string, { command: AgentCanvasCommand; resolve: (approved: boolean) => void }>();
  ...
  async execute(command: AgentCanvasCommand): Promise<AgentCanvasResponse> {
    ...
    if (!this.policy.allowedCommands.includes(command.type)) {
      return this.error('COMMAND_NOT_ALLOWED', `Command ${command.type} not permitted for this agent`);
    }
  ...
  private requestConfirmation(confirmationId: string, command: AgentCanvasCommand): Promise<boolean> {
  ...
  private async handleCreateNode(cmd: Extract<AgentCanvasCommand, { type: 'canvas.createNode' }>): Promise<AgentCanvasResponse> {
    ...
    if (providerId && !this.policy.allowedProviders.includes(providerId)) {
      return this.error('PROVIDER_NOT_ALLOWED', `Provider ${providerId} not permitted for this agent`);
    }
  ...
  private async handleDeleteNode(cmd: Extract<AgentCanvasCommand, { type: 'canvas.deleteNode' }>): Promise<AgentCanvasResponse> {
    const { nodeId } = cmd.payload;
    const bus = getCanvasEventBus();
    bus.emit(CanvasEventType.AGENT_DELETE_NODE, { nodeId, agentId: this.agentId, workspaceId: this.workspaceId });
    return { type: 'canvas.nodeDeleted', payload: { nodeId } };
  }

  private async handleMoveNode(cmd: Extract<AgentCanvasCommand, { type: 'canvas.moveNode' }>): Promise<AgentCanvasResponse> {
    const { nodeId, x, y } = cmd.payload;
    const bus = getCanvasEventBus();
    bus.emit(CanvasEventType.AGENT_MOVE_NODE, { nodeId, x, y, agentId: this.agentId, workspaceId: this.workspaceId });
    return { type: 'canvas.nodesMoved', payload: [{ nodeId, x, y }] };
  }

  private async handleConnectNodes(cmd: Extract<AgentCanvasCommand, { type: 'canvas.connectNodes' }>): Promise<AgentCanvasResponse> {
    const { fromNodeId, toNodeId } = cmd.payload;
    const bus = getCanvasEventBus();
    bus.emit(CanvasEventType.AGENT_CONNECT_NODES, { fromNodeId, toNodeId, agentId: this.agentId, workspaceId: this.workspaceId });
    return { type: 'canvas.nodesConnected', payload: { fromNodeId, toNodeId } };
  }

  private async handleDisconnectNodes(cmd: Extract<AgentCanvasCommand, { type: 'canvas.disconnectNodes' }>): Promise<AgentCanvasResponse> {
    const { fromNodeId, toNodeId } = cmd.payload;
    const bus = getCanvasEventBus();
    bus.emit(CanvasEventType.AGENT_DISCONNECT_NODES, { fromNodeId, toNodeId, agentId: this.agentId, workspaceId: this.workspaceId });
    return { type: 'canvas.nodesDisconnected', payload: { fromNodeId, toNodeId } };
  }

  private async handleRunLayout(cmd: Extract<AgentCanvasCommand, { type: 'canvas.runLayout' }>): Promise<AgentCanvasResponse> {
    const { intent, params } = cmd.payload;
    const bus = getCanvasEventBus();
    bus.emit(CanvasEventType.AGENT_RUN_LAYOUT, { intent, params, agentId: this.agentId, workspaceId: this.workspaceId });
    return { type: 'canvas.layoutApplied', payload: { intent, positions: {} } };
  }

  private async handleStartStream(cmd: Extract<AgentCanvasCommand, { type: 'canvas.startStream' }>): Promise<AgentCanvasResponse> {
    ...
    if (activeStreams >= this.policy.maxConcurrentStreams) {
      return this.error('STREAM_LIMIT_EXCEEDED', `Max ${this.policy.maxConcurrentStreams} concurrent streams allowed`);
    }

    bus.emit(CanvasEventType.AGENT_START_STREAM, { nodeId, capabilityId, input, agentId: this.agentId, workspaceId: this.workspaceId });
    const sessionId = `stream:${ulid()}`;
    return { type: 'canvas.streamStarted', payload: { nodeId, sessionId } };
  }

  private async handleStopStream(cmd: Extract<AgentCanvasCommand, { type: 'canvas.stopStream' }>): Promise<AgentCanvasResponse> {
    const { nodeId } = cmd.payload;
    const bus = getCanvasEventBus();
    bus.emit(CanvasEventType.AGENT_STOP_STREAM, { nodeId, agentId: this.agentId, workspaceId: this.workspaceId });
    return { type: 'canvas.streamStopped', payload: { nodeId } };
  }

  private async handleSetViewport(cmd: Extract<AgentCanvasCommand, { type: 'canvas.setViewport' }>): Promise<AgentCanvasResponse> {
    const { x, y, zoom } = cmd.payload;
    const bus = getCanvasEventBus();
    bus.emit(CanvasEventType.AGENT_SET_VIEWPORT, { x, y, zoom, agentId: this.agentId, workspaceId: this.workspaceId });
    return { type: 'canvas.state', payload: await this.getCurrentState() };
  }

  private async handleFocusNode(cmd: Extract<AgentCanvasCommand, { type: 'canvas.focusNode' }>): Promise<AgentCanvasResponse> {
    const { nodeId } = cmd.payload;
    const bus = getCanvasEventBus();
    bus.emit(CanvasEventType.AGENT_FOCUS_NODE, { nodeId, agentId: this.agentId, workspaceId: this.workspaceId });
    return { type: 'canvas.state', payload: await this.getCurrentState() };
  }

  private async handleGetState(cmd: Extract<AgentCanvasCommand, { type: 'canvas.getState' }>): Promise<AgentCanvasResponse> {
    return { type: 'canvas.state', payload: await this.getCurrentState() };
  }
