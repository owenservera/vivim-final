...
import type { AgentCanvasCommand, AgentCanvasPolicy, AgentCanvasResponse } from '@/shared/agent-canvas';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { agentId, workspaceId, command } = body as { agentId: string; workspaceId: string; command: AgentCanvasCommand };

    if (!agentId || !workspaceId || !command) {
      return NextResponse.json({ error: 'Missing agentId, workspaceId, or command' }, { status: 400 });
    }
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const body = await req.json();
    const { agentId, workspaceId, policy } = body as { agentId: string; workspaceId: string; policy: Partial<AgentCanvasPolicy> };
