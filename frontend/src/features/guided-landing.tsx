...
const PROVIDERS: ProviderChip[] = [
  { id: 'chatgpt',  name: 'ChatGPT',  desc: 'OpenAI' },
  { id: 'claude',   name: 'Claude',   desc: 'Anthropic' },
  { id: 'gemini',   name: 'Gemini',   desc: 'Google' },
  { id: 'deepseek', name: 'DeepSeek', desc: 'DeepSeek' },
  { id: 'grok',     name: 'Grok',     desc: 'xAI' },
  { id: 'qwen',     name: 'Qwen',     desc: 'Alibaba' },
];
...
function useTypewriter(text: string, opts?: { cps?: number; onDone?: () => void; enabled?: boolean }) {
...
function MinimalComposer({ value, onChange, onSubmit, disabled, placeholder, autoFocus }: ComposerProps) {
  ...
  return (
    <div
      ...
      onFocus={(e) => {
        e.currentTarget.style.borderColor = 'color-mix(in oklch, var(--foreground) 25%, var(--border))';
      }}
      onBlur={(e) => {
        e.currentTarget.style.borderColor = 'var(--border)';
      }}
...
export function GuidedLanding({ isOpen, mode = 'onboarding', onClose, onComplete }: GuidedLandingProps) {
  ...
  const pushAgent = useCallback((text: string, opts?: { chips?: ProviderChip[]; chipsDisabled?: boolean }) => {
  ...
  const waitForMessageTyped = useCallback((id: string, timeoutMs = 8000): Promise<void> => {
    return new Promise((resolve) => {
      ...
      const check = () => {
        ...
        if (found?.fullyTyped) { resolve(); return; }
        if (Date.now() - start > timeoutMs) { resolve(); return; } // safety timeout
        setTimeout(check, 60);
      };
      check();
    });
  ...
  const runOnboardingFlow = useCallback(async (provider: ProviderChip) => {
    setState('launching_chrome');
    const launchMsgId = pushAgent(`Opening ${provider.name} in a Chrome window so you can log in. I'll watch for it — log in there and come back here when you're done.`);
    ...
    try {
        ...
        try {
          ...
          if (attempts > 100 && pollRef.current) {
            ...
            if (mountedRef.current) {
              setState('error');
              pushAgent('I lost the connection to Chrome. Refresh the page and we\'ll try again.');
            }
          }
        }
      }, 3000);
      ...
      pushAgent(`I couldn't open Chrome: ${msg}. You can try again — refresh the page.`);
    }
  }, [pushAgent, onComplete, waitForMessageTyped]);

  // ── Provider pick handler (chip-click path) ──────────────────────────────

  const handleProviderPicked = useCallback((provider: ProviderChip) => {
    ...
    setMessages((prev) => prev.map((m) =>
      m.chips && m.chips.length > 0 ? { ...m, chipsDisabled: true } : m,
    ));
  ...
  const handleSubmit = useCallback((text: string) => {
    ...
    if (stateRef.current === 'awaiting_provider_pick') {
      pushAgent("I didn't catch a provider name. Pick one of the chips above, or type its name (e.g. \"ChatGPT\").");
    } else if (stateRef.current === 'awaiting_login') {
      pushAgent('Still watching for your login in the Chrome window — once you log in there, I\'ll detect it.');
      ...
      >
        ...
        {(state === 'launching_chrome' || state === 'awaiting_login' || state === 'completing') && (
          ...
          >
            ...
            {state === 'awaiting_login' && 'watching for login…'}
            {state === 'completing' && 'finalizing setup…'}
          </div>
        )}
      </div>
...
function detectProvider(text: string): string | null {
  // Strip filler words
  const cleaned = text
    .replace(/^(i\s+want\s+to\s+use|i\s+want\s+to|use|i\s+want|let'?s\s+use|let'?s|pick|choose|go\s+with|try)\s+/i, '')
    .trim();
