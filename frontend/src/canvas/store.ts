...
import { type CanvasConfig, DEFAULT_CANVAS_CONFIG, mergeConfigs } from './config'
import type { CanvasBookmark, CanvasEdge, CanvasNode, CanvasState, Viewport } from './types'
...
export const useCanvasStore = create<CanvasStore>((set, _get) => ({
  ...
  zoomBy: (factor, anchor) =>
    set((s) => {
      const vp = s.state.viewport
      const newScale = Math.max(s.config.zoom.min, Math.min(s.config.zoom.max, vp.scale * factor))
  ...
  fitToContent: (padding = 100) =>
    set((s) => {
      ...
      const xs = s.state.nodes.flatMap((n) => [n.position.x, n.position.x + (n.size?.x ?? 0)])
      const ys = s.state.nodes.flatMap((n) => [n.position.y, n.position.y + (n.size?.y ?? 0)])
  ...
  setConfig: (patch) => set((s) => ({ config: mergeConfigs(s.config, patch) })),
