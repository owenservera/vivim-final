...
import type { AccountContext, ResolvedSurface } from '../../shared/route-context'
...
export function LiveConfigProvider(props: LiveConfigProviderProps) {
  ...
  const [providerIds, setProviderIds] = useState(props.initialProviderIds ?? ['chatgpt'])
  const [accounts, setAccounts] = useState(
    props.initialAccounts ?? [
      { accountId: 'acct:chatgpt:free', providerId: 'chatgpt', planTier: 'free' as const },
    ],
  )
  ...
  const patchDefinition = useCallback(
    async (id: string, patch: Record<string, unknown>) => {
      await fetch(getApiUrl(`/api/canvas/definition/${id}`), {
        method: 'PATCH',
