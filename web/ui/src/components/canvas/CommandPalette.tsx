...
import { listCapabilities, executeCapability } from '../../sdk/backend-client';

const KIND_GROUPS: Array<{ kind: SearchEntityKind; label: string; icon: string }> = [
  { kind: 'command', label: 'Commands', icon: '⌨️' },
  { kind: 'workspace', label: 'Workspaces', icon: '🗂️' },
...
// Convert the SDK capability list response into palette SearchHits (Moment 5).
function toCapabilityHits(res: { ok: boolean; data?: { capabilities: Array<{ id: string; slug: string; name: string; description?: string }> } }): SearchHit[] {
  if (!res.ok || !res.data) return [];
  return res.data.capabilities.map((c) => ({
    kind: 'capability' as SearchEntityKind,
    id: c.id,
    title: c.name,
    subtitle: c.description ?? c.slug,
    score: 0,
    icon: '🛠️',
    actionUrl: `capability:${c.id}`,
    actionLabel: 'execute',
  }));
}
...
export function CommandPalette({ open, onClose, onAction, workspaceId }: CommandPaletteProps) {
  ...
  // Focus input on open + load CLI capabilities as the base command list (Moment 5).
  useEffect(() => {
    if (open) {
      setQuery('');
      setSelectedIndex(0);
      ...
      let cancelled = false;
      setLoading(true);
      listCapabilities('cli')
        .then((resp) => {
          if (cancelled) return;
          setHits(toCapabilityHits(resp));
        })
        .catch(() => {
  ...
  // Debounced search across capabilities (Moment 5 AC2: filtering).
  useEffect(() => {
    if (!open) return;
    if (query.trim().length < 1) {
      // Re-load the full capability list when the query clears.
      let cancelled = false;
      setLoading(true);
      listCapabilities('cli')
        .then((resp) => {
    ...
    const t = setTimeout(async () => {
      try {
        const resp = await listCapabilities('cli');
        const q = query.trim().toLowerCase();
        const all = toCapabilityHits(resp);
        const filtered = all.filter((h) =>
          h.title.toLowerCase().includes(q) ||
          (h.subtitle?.toLowerCase().includes(q) ?? false),
        );
        setHits(filtered);
        setSelectedIndex(0);
      } catch {
