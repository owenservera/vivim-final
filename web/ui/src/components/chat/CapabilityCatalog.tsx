...
export function CapabilityCatalog() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(getApiUrl('/api/capabilities?surface=ui'));
      const data = await res.json();
      setCaps(data.capabilities ?? data ?? []);
    } catch (e: unknown) {
      setError(String(e));
    } finally {
      setLoading(false);
  ...
  const execute = async (cap: Capability) => {
    ...
    setResult(null);
    try {
      const res = await fetch(getApiUrl(`/api/capabilities/${cap.id}/execute`), {
        method: 'POST',
      ...
      setResult(`${cap.label}: ${ok ? 'OK' : 'failed'} — ${JSON.stringify(data).slice(0, 200)}`);
