...
// ConversationSurface — chat interface with streaming block delivery.
// Renders message history, handles send, and receives real-time updates via WebSocket.
// Phase 5: Optimistic UI, RAF batching, virtual scrolling, latency display.
...
export function ConversationSurface({ conversationId }: ConversationSurfaceProps) {
  ...
  const handleSend = useCallback(async () => {
    ...
    // INSTANT: Show user message immediately
    ...
    try {
      ...
      if (result.ok) {
        ...
        // If server returned a result immediately (non-streaming), add it
        ...
        // Otherwise, assistant message arrives via WebSocket streaming
      } else {
        // FAILED: Remove optimistic messages
