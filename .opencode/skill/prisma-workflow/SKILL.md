...
## Node-Layer v2 Migrations

The universal Node layer (`Node` + `NodeEdge` + `NodeVersion` + `NodeAlias`) lives in `prisma/schema.prisma`. Documentation: `docs/node-layer-v2/`.

### Key Models

| Model | Purpose | Key Fields |
|-------|---------|------------|
| `Node` | Universal entity | `type`, `contentHash`, `version`, `state`, `securityLevel`, `authorDid`, `aclJson`, `qualityJson`, `validFrom`, `validUntil` |
| `NodeEdge` | Materialized graph | `sourceId`, `targetId`, `edgeType`, `weight` |
| `NodeVersion` | Time-travel | `nodeId`, `version`, `op`, `parentVersion` |
| `NodeAlias` | Entity merge | `aliasId`, `canonicalId`, `method`, `confidence` |

### Migration Pattern

Use **additive-only** changes (ALTER TABLE ADD COLUMN, CREATE TABLE). SQLite does not support DROP COLUMN or complex ALTER.

```bash
# 1. Edit prisma/schema.prisma
# 2. Sync DB (skips migration dir, just pushes schema)
DATABASE_URL="file:./dev.db" bunx prisma db push --skip-generate
# 3. Regenerate client
bunx prisma generate
# 4. Create migration SQL in prisma/migrations/<timestamp>_<name>/migration.sql
# 5. Record in migration_log (see scripts/_record_node_migration.ts)
```
...
const p = getPrisma()
const sql = readFileSync('prisma/migrations/<timestamp>_<name>/migration.sql', 'utf-8')
const checksum = await Bun.CryptoHasher.hash('sha256', sql, 'hex')

if (!(await p.migrationLog.findFirst({ where: { filename: '<timestamp>_<name>/migration.sql' } }))) {
  await p.migrationLog.create({
    data: { id: newId(), filename: '<timestamp>_<name>/migration.sql', checksum, appliedAt: Date.now() },
  })
}
await p.$disconnect()
```

### Fixture DB for Tests
After schema changes, rebuild the Node-store test fixture:
```bash
DATABASE_URL="file:./tests/fixtures/node-store-test.db" bunx prisma db push --skip-generate --accept-data-loss
```

This keeps `tests/fixtures/node-store-test.db` in sync with the current schema.

### Migration History
| Migration | Description |
|-----------|-------------|
| `20260718022736_universal_node_layer` | Base Node + NodeEdge tables |
| `20260718041000_node_layer_v2` | ACU fields + NodeVersion + NodeAlias |