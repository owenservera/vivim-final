...
export function snapshotState(
  ...
  const nodes = doc.getArray<Y.Map<unknown>>("nodes").map(
    (m) => m.toJSON() as unknown as CanvasNode,
  );
  const edges = doc.getArray<Y.Map<unknown>>("edges").map(
    (m) => m.toJSON() as unknown as CanvasEdge,
  );
  return {
