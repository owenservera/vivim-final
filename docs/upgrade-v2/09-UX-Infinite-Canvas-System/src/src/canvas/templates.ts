...
import type { CanvasState, CanvasNode, CanvasEdge, Vec2 } from "./types";
...
export const TEMPLATES: Template[] = [
  ...
  },
  {
    ...
    description: "Horizontal lanes per team (security, eng, design, ops). Packages in their owner's lane.",
    apply: (packages) => {
      const lanes = [
      ...
      const nodes: CanvasNode[] = packages.map((pkg) => {
        const laneIdx = lanes.findIndex((l) =>
          l.ownerCategories.includes(pkg.category),
        );
