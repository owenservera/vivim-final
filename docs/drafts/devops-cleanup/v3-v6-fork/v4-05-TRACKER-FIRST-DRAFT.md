# vivim-final v4-fork-canon — Atomic Tracker (CDP/Chrome Execution)

**Total units:** 71 | **Done:** 0 | **Blocked:** 0 | **Pending:** 71

> The authoritative source of truth for v4-fork-canon implementation progress.
> States: `[ ]` pending · `[~]` in_progress · `[x]` done · `[!]` blocked.
> Update via `bun run devops mark <id> <state>`.

> **v4 Philosophy:** v4 is user-journey driven — each phase is a vertical slice
> that delivers demonstrable working functionality. Zero-breakage E2E from frontend
> login through multi-turn conversation with ChatGPT, Claude, and Gemini.

> **CDP/Chrome Execution Path:** This fork uses Chrome DevTools Protocol (CDP) to
> control Chrome browsers for conversations. No kernel layer — that's v5.

> **Re-Programmability Principle:** Every design uses configurable policy objects
> loaded from DB — never hardcoded constants.

## Phase 1: E2E Bootstrap & Login (7 units)

- [ ] 1.1 — Wire CDP transport into ChromeGovernor bootstrap → `docs/atomic-v4/phase-01-e2e-bootstrap/1.1-wire-cdp-transport.md`
- [ ] 1.2 — Provider seed pipeline: ensure chatgpt/claude/gemini seeds loaded at boot → `docs/atomic-v4/phase-01-e2e-bootstrap/1.2-seed-pipeline.md`
- [ ] 1.3 — Setup wizard workspace + profile path flow → `docs/atomic-v4/phase-01-e2e-bootstrap/1.3-workspace-profile-flow.md`
- [ ] 1.4 — Launch visible Chrome with correct profile for login → `docs/atomic-v4/phase-01-e2e-bootstrap/1.4-visible-chrome-login.md`
- [ ] 1.5 — CDP-based login state verification → `docs/atomic-v4/phase-01-e2e-bootstrap/1.5-login-verify.md`
- [ ] 1.6 — Complete setup: persist account with profile + port → `docs/atomic-v4/phase-01-e2e-bootstrap/1.6-complete-persist.md`
- [ ] 1.7 — Headless slave reuse of saved login profile → `docs/atomic-v4/phase-01-e2e-bootstrap/1.7-headless-profile-reuse.md`

## Phase 2: Single-Turn Conversation (8 units)

- [ ] 2.1 — Fix slaveId derivation: match FleetSupervisor naming → `docs/atomic-v4/phase-02-single-turn/2.1-slave-id-derivation.md`
- [ ] 2.2 — Implement HarnessRuntime.executeHarnessPlan (not stub) → `docs/atomic-v4/phase-02-single-turn/2.2-harness-real-exec.md`
...
- [ ] 2.6 — Parser: SSE/streaming body → ContentBlock[] → `docs/atomic-v4/phase-02-single-turn/2.6-parser-extract.md`
- [ ] 2.7 — Store message + blocks + emit events → `docs/atomic-v4/phase-02-single-turn/2.7-store-emit.md`
