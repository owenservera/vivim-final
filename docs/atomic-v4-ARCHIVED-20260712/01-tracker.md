# vivim-final v4 — Atomic Tracker (SUPERSEDED)

> **⚠️ SUPERSEDED — DO NOT IMPLEMENT FROM THIS TRACKER**
>
> This tracker has been archived. **`docs/atomic-v4-fork-canon/01-tracker.md`** (71 units) is the MASTER.
> All 71 units are done.
>
> **ACTIVE TRACKERS (for implementation):**
> - `docs/atomic-v3-fork-canon/01-tracker.md` — MASTER source of truth (127 units, 12 done)
> - `docs/atomic-v4-fork-canon/01-tracker.md` — CDP/Chrome subset (71 units, all done) — **USE THIS FOR CDP/Chrome work**
...
**Total units:** 71 | **Done:** 15 | **Blocked:** 0 | **Pending:** 56

> **v4 Philosophy:** v3 was abstract and phase-gated. v4 is user-journey driven —
> each phase is a vertical slice that delivers demonstrable working functionality.
> The goal: zero-breakage E2E from frontend login through multi-turn conversation
> with ChatGPT, Claude, and Gemini, optimized for frontend UI performance.

> **Re-Programmability Principle:** Every design uses configurable policy objects
> loaded from DB — never hardcoded constants, never single-solution patterns.
> Retry counts, TTLs, fleet limits, log formats, and launch profiles are all
> runtime-configurable via the ConfigManager + policy store system.

## Phase 1: E2E Bootstrap & Login (7 units)

- [x] 1.1 — Wire CDP transport into ChromeGovernor bootstrap → `docs/atomic-v4/phase-01-e2e-bootstrap/1.1-wire-cdp-transport.md`
- [x] 1.2 — Provider seed pipeline: ensure chatgpt/claude/gemini seeds loaded at boot → `docs/atomic-v4/phase-01-e2e-bootstrap/1.2-seed-pipeline.md`
- [x] 1.3 — Setup wizard workspace + profile path flow → `docs/atomic-v4/phase-01-e2e-bootstrap/1.3-workspace-profile-flow.md`
- [x] 1.4 — Launch visible Chrome with correct profile for login → `docs/atomic-v4/phase-01-e2e-bootstrap/1.4-visible-chrome-login.md`
- [x] 1.5 — CDP-based login state verification → `docs/atomic-v4/phase-01-e2e-bootstrap/1.5-login-verify.md`
- [x] 1.6 — Complete setup: persist account with profile + port → `docs/atomic-v4/phase-01-e2e-bootstrap/1.6-complete-persist.md`
- [x] 1.7 — Headless slave reuse of saved login profile → `docs/atomic-v4/phase-01-e2e-bootstrap/1.7-headless-profile-reuse.md`

## Phase 2: Single-Turn Conversation (8 units)

- [x] 2.1 — Fix slaveId derivation: match FleetSupervisor naming → `docs/atomic-v4/phase-02-single-turn/2.1-slave-id-derivation.md`
- [x] 2.2 — Implement HarnessRuntime.executeHarnessPlan (not stub) → `docs/atomic-v4/phase-02-single-turn/2.2-harness-real-exec.md`
- [x] 2.3 — Provider-specific composer typing via CDP → `docs/atomic-v4/phase-02-single-turn/2.3-composer-typing.md`
- [x] 2.4 — Provider-specific submit action via CDP → `docs/atomic-v4/phase-02-single-turn/2.4-submit-action.md`
- [x] 2.5 — Network capture: intercept streaming API response → `docs/atomic-v4/phase-02-single-turn/2.5-network-capture.md`
- [x] 2.6 — Parser: SSE/streaming body → ContentBlock[] → `docs/atomic-v4/phase-02-single-turn/2.6-parser-extract.md`
- [x] 2.7 — Store message + blocks + emit events → `docs/atomic-v4/phase-02-single-turn/2.7-store-emit.md`
- [x] 2.8 — Frontend: render single response in conversation surface → `docs/atomic-v4/phase-02-single-turn/2.8-frontend-render.md`

## Phase 3: Multi-Turn Conversation (6 units)

- [ ] 3.1 — Conversation state persistence across turns → `docs/atomic-v4/phase-03-multi-turn/3.1-state-persistence.md`
- [ ] 3.2 — DOM recovery: page reload + SPA navigation handling → `docs/atomic-v4/phase-03-multi-turn/3.2-dom-recovery.md`
- [ ] 3.3 — Streaming: progressive block delivery over WebSocket → `docs/atomic-v4/phase-03-multi-turn/3.3-streaming-ws.md`
- [ ] 3.4 — Frontend: message list with live streaming updates → `docs/atomic-v4/phase-03-multi-turn/3.4-frontend-streaming.md`
- [ ] 3.5 — Error recovery: slave crash + circuit breaker mid-conversation → `docs/atomic-v4/phase-03-multi-turn/3.5-error-recovery.md`
- [ ] 3.6 — Selector healing: auto-detect + repair broken selectors → `docs/atomic-v4/phase-03-multi-turn/3.6-selector-healing.md`
...
- [ ] 4.5 — Provider health monitoring for all three providers → `docs/atomic-v4/phase-04-three-provider/4.5-health-monitor.md`
