...
fn extract_port_from_line(line: &str) -> Option<u16> {
    ...
    // Skip host part, find the colon before port
        ...
        while let Some(event) = rx.recv().await {
            match event {
                ...
                CommandEvent::Stderr(bytes) => {
                    let line = String::from_utf8_lossy(&bytes);
                    eprintln!("[vivim-server:err] {line}");
                }
        ...
        for attempt in 0..=MAX_RESTARTS {
            ...
            match spawn_backend(&app_clone, current_port) {
                Ok((port, exit_rx)) => {
                    ...
                    // Wait for sidecar to exit (it crashes or shuts down)
                    let _ = exit_rx.await;
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            tauri::async_runtime::spawn(async move {
                tokio::time::sleep(tokio::time::Duration::from_millis(BACKEND_READY_TIMEOUT_MS)).await;
                if !win_timeout.is_visible().unwrap_or(false) {
                    eprintln!("[vivim] WARNING: backend-ready timeout after {BACKEND_READY_TIMEOUT_MS}ms, showing window anyway");
