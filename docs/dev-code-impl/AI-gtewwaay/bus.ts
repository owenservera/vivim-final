...
import type { AIError, ProviderHealth, ProviderId, ProviderState, ResourceUsage } from '../core/types';
import type { ExecutionEvent } from '../execution/types';

export type ProviderLifecycleEvent =
  | { readonly type: 'provider.state-changed'; readonly providerId: ProviderId; readonly from: ProviderState; readonly to: ProviderState; readonly at: string }
  | { readonly type: 'provider.health-changed'; readonly providerId: ProviderId; readonly health: ProviderHealth }
  | { readonly type: 'provider.crashed'; readonly providerId: ProviderId; readonly error: AIError };

export type ResourceEvent =
  | { readonly type: 'resource.pressure'; readonly kind: 'ram' | 'vram' | 'cpu' | 'disk'; readonly usage: ResourceUsage; readonly at: string }
  | { readonly type: 'resource.lease-denied'; readonly providerId: ProviderId; readonly reason: string; readonly at: string };
