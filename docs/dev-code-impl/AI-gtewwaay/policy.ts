...
import type {
  AIErrorCode, AIRequest, ExecutionPolicy, ModelDescriptor, ProviderManifest, RoutingCandidate, ToolDefinition,
} from '../core/types';
...
export interface IPolicyEvaluator {
  ...
  scoreCandidates(request: AIRequest, candidates: readonly CandidateInput[]): Promise<readonly RoutingCandidate[]>;
...
export interface IPolicyEnforcer {
  ...
  enforcePersistencePolicy(policy: ExecutionPolicy, operation: 'read' | 'write' | 'delete'): Promise<PolicyDecision>;

  /** Hard gate for telemetry/prompt-logging export — must default deny per the privacy-preserving-telemetry principle. */
  enforceTelemetryPolicy(policy: ExecutionPolicy, payloadKind: 'metadata' | 'prompt' | 'output'): Promise<PolicyDecision>;
}
