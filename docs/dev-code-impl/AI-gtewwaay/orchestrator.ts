...
import type { ToolCallContent, ToolDefinition, WorkspaceId } from '../core/types';
...
export interface IToolAuthorizer {
  authorize(tool: ToolDefinition, call: ToolCallContent, context: AuthorizationContext): Promise<AuthorizationDecision>;
}
...
export interface IApprovalManager {
  requestApproval(tool: ToolDefinition, call: ToolCallContent, context: AuthorizationContext): Promise<boolean>;
...
export interface IToolOrchestrator {
  ...
  handle(tool: ToolDefinition, call: ToolCallContent, context: AuthorizationContext, signal?: AbortSignal): Promise<unknown>;
}
