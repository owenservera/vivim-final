...
export type OutputModality =
  | 'text' | 'image' | 'audio' | 'video' | 'structured-data' | 'embedding' | 'tool-call';
...
export type AICapability =
  | 'chat' | 'generation' | 'streaming' | 'reasoning' | 'vision'
  | 'audio-input' | 'audio-output' | 'video-input'
  | 'tool-calling' | 'parallel-tool-calls'
  | 'structured-output' | 'embeddings' | 'reranking' | 'image-generation'
  | 'code-execution' | 'json-schema' | 'grammar-constrained-output'
  | 'cancellation' | 'usage-reporting' | 'prompt-caching' | 'context-caching';
...
export type ProviderState =
  | 'discovered' | 'installed' | 'validating' | 'enabled' | 'starting'
  | 'ready' | 'active' | 'degraded' | 'unhealthy'
  | 'draining' | 'disabled' | 'stopped' | 'failed';

export type Platform = 'windows' | 'macos' | 'linux' | 'web';
export type Architecture = 'x64' | 'arm64' | 'x86';

export type ProviderPermission =
  | 'filesystem.models' | 'filesystem.config'
  | 'network.localhost' | 'network.specific-hosts' | 'network.internet'
  | 'gpu' | 'process.spawn' | 'microphone' | 'camera' | 'credentials';
...
export type ContentPart =
  ...
  | DataContent | ToolCallContent | ToolResultContent | CitationContent;

export interface TextContent { readonly type: 'text'; readonly text: string }
...
export interface ImageContent { readonly type: 'image'; readonly source: MediaSource }
export interface AudioContent { readonly type: 'audio'; readonly source: MediaSource }
...
export interface DataContent { readonly type: 'data'; readonly mediaType: string; readonly data: unknown }
...
export function isTextContent(c: ContentPart): c is TextContent { return c.type === 'text'; }
export function isToolCallContent(c: ContentPart): c is ToolCallContent { return c.type === 'tool-call'; }
export function isToolResultContent(c: ContentPart): c is ToolResultContent { return c.type === 'tool-result'; }
...
export type ToolPermission =
  ...
  | 'browser' | 'credentials' | 'external-service';
...
export type ResponseFormat =
  | { readonly type: 'text' }
  | { readonly type: 'json' }
  | { readonly type: 'json-schema'; readonly name: string; readonly schema: JsonSchema; readonly strict?: boolean };
...
export type LocalityPolicy =
  | 'local-only' | 'local-preferred' | 'local-with-fallback' | 'remote-preferred' | 'remote-only';
...
export interface RequestStartedEvent extends BaseAIEvent { readonly type: 'request.started' }
export interface ResponseStartedEvent extends BaseAIEvent {
  readonly type: 'response.started'; readonly providerId: ProviderId; readonly modelId: ModelId;
}
export interface TextDeltaEvent extends BaseAIEvent { readonly type: 'output.text.delta'; readonly text: string }
export interface ReasoningDeltaEvent extends BaseAIEvent { readonly type: 'output.reasoning.delta'; readonly text: string }
export interface ToolCallCreatedEvent extends BaseAIEvent {
  readonly type: 'tool.call.created'; readonly toolCallId: ToolCallId; readonly name: string;
}
export interface ToolCallDeltaEvent extends BaseAIEvent {
  readonly type: 'tool.call.delta'; readonly toolCallId: ToolCallId; readonly argumentsDelta: string;
}
export interface ToolCallCompletedEvent extends BaseAIEvent {
  readonly type: 'tool.call.completed'; readonly toolCallId: ToolCallId; readonly arguments: unknown;
}
export interface StructuredOutputDeltaEvent extends BaseAIEvent { readonly type: 'output.structured.delta'; readonly data: unknown }
export interface UsageUpdatedEvent extends BaseAIEvent { readonly type: 'usage.updated'; readonly usage: UsageMetrics }
export interface ProviderWarningEvent extends BaseAIEvent {
  readonly type: 'provider.warning'; readonly code: string; readonly message: string;
}
export interface ResponseCompletedEvent extends BaseAIEvent { readonly type: 'response.completed'; readonly usage?: UsageMetrics }
export interface ResponseFailedEvent extends BaseAIEvent { readonly type: 'response.failed'; readonly error: AIError }
export interface ResponseCancelledEvent extends BaseAIEvent { readonly type: 'response.cancelled'; readonly reason?: string }

export type AIEvent =
  | RequestStartedEvent | ResponseStartedEvent | TextDeltaEvent | ReasoningDeltaEvent
  | ToolCallCreatedEvent | ToolCallDeltaEvent | ToolCallCompletedEvent
  | StructuredOutputDeltaEvent | UsageUpdatedEvent | ProviderWarningEvent
  | ResponseCompletedEvent | ResponseFailedEvent | ResponseCancelledEvent;

export function isTerminalAIEvent(
  e: AIEvent,
): e is ResponseCompletedEvent | ResponseFailedEvent | ResponseCancelledEvent {
  return e.type === 'response.completed' || e.type === 'response.failed' || e.type === 'response.cancelled';
}
...
export type AIErrorCode =
  | 'PROVIDER_UNAVAILABLE' | 'PROVIDER_UNHEALTHY' | 'MODEL_UNAVAILABLE'
  | 'CAPABILITY_UNSUPPORTED' | 'POLICY_DENIED' | 'AUTHENTICATION_FAILED'
  | 'RATE_LIMITED' | 'CONTEXT_TOO_LARGE' | 'INVALID_REQUEST' | 'SCHEMA_VIOLATION'
  | 'TOOL_DENIED' | 'TOOL_FAILED' | 'CANCELLED' | 'TIMEOUT' | 'RESOURCE_UNAVAILABLE'
  | 'PLUGIN_INVALID' | 'PLUGIN_UNTRUSTED' | 'RUNTIME_CRASH' | 'HARDWARE_UNAVAILABLE'
...
export type RoutingFactor =
  | 'capability' | 'locality' | 'policy' | 'resource' | 'latency' | 'quality' | 'availability' | 'cost';
...
export type PluginState =
  | 'discovered' | 'installing' | 'installed' | 'validating'
